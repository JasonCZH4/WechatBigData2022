...
class UniBert(BertPreTrainedModel):
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, gather_index=None):
        ...
        last4_hidden = \
        self.encoder(fusion_emb, attention_mask=fusion_mask_attention, output_hidden_states=True)['hidden_states'][-4]
        ...
        # cat
        embed_mean = self.cls_ln(
            torch.cat((embed_mean_fusion, embed_mean_hidden, pooler_outputs, attention), dim=1))  # (bs, 3072)
        return embed_mean
