...
from src.simple.data_helpers.data_helper import create_dataloaders
from utils import EMA, FGM
from models.model import QQModel3
...
def swa(model, model_dir, swa_start=1):
    ...
    with torch.no_grad():
        for _ckpt in model_path_list[swa_start:4]:
...
def train_and_validate(args):
    ...
    model = QQModel3(args, model_path=args.bert_dir)
    swa_raw_model = copy.deepcopy(model)
    optimizer, scheduler = build_optimizer(args, model)
    checkpoint = torch.load(args.pretrain_load_path, map_location='cpu')
    ...
    for epoch in range(args.max_epochs):
        ...
        # 5. save checkpoint
        if not os.path.exists(args.savedmodel_FGM_path):
            os.mkdir(args.savedmodel_FGM_path)
        # mean_f1 = results['mean_f1']
        # if mean_f1 > best_score:
        state_dict = model.module.state_dict() if args.device == 'cuda' else model.state_dict()
        torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'loss': loss},
                   f'{args.savedmodel_FGM_path}/model_epoch_{epoch}.bin')
        ema.restore()
    swa(swa_raw_model, args.savedmodel_FGM_path, swa_start=1)
...
def main():
    ...
    os.makedirs(args.savedmodel_FGM_path, exist_ok=True)
