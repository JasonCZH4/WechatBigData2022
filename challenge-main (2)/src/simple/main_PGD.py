...
def swa(model, model_dir, swa_start=1):
    ...
    with torch.no_grad():
        for _ckpt in model_path_list[swa_start:3]:
...
def train_and_validate(args):
    ...
    for epoch in range(args.max_epochs):
        ...
        # 5. save checkpoint
        if not os.path.exists(args.savedmodel_PGD_path):
            os.mkdir(args.savedmodel_PGD_path)
        # mean_f1 = results['mean_f1']
        # if mean_f1 > best_score:
        state_dict = model.module.state_dict() if args.device == 'cuda' else model.state_dict()
        torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'loss': loss},
                   f'{args.savedmodel_PGD_path}/model_epoch_{epoch}.bin')
        ema.restore()
    swa(swa_raw_model, args.savedmodel_PGD_path, swa_start=1)
...
def main():
    ...
    os.makedirs(args.savedmodel_PGD_path, exist_ok=True)
